...
def convbn(in_planes, out_planes, kernel_size, stride, pad, dilation, gn=False, groups=32, deconv=False):
    if not deconv:
        ...
        return nn.Sequential(
            nn.ConvTranspose2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=pad, output_padding=stride-1, dilation=dilation, bias=False),
            nn.BatchNorm2d(out_planes) if not gn else nn.GroupNorm(groups, out_planes)
        )
...
class feature_extraction(nn.Module):
    def __init__(self, cfg):
        ...
        self.firstconv = nn.Sequential(convbn(1, first_dim, 3, 2, 1, 1, gn=cfg.GN if first_dim >= 32 else False),
                                       nn.ReLU(inplace=True),
