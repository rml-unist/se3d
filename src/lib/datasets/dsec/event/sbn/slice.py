...
def load_events_with_retry(event_root, t_start_us_idx, t_end_us_idx, t_offset, max_retries=10):
    """
    ...
    while attempts < max_retries:
        try:
            with h5py.File(event_root, 'r') as h5f:
                events = {'t': np.asarray(h5f['events/t'][t_start_us_idx:t_end_us_idx]) + t_offset}
...
def load_time_with_retry(event_root, t_start_ms_idx, t_end_ms_idx, max_retries=10):
    """
    Load event data with retries in case of an IOError.
    ...
    while attempts < max_retries:
        try:
            with h5py.File(event_root, 'r') as h5f:
                ...
                return time_array_conservative
...
class EventSlicer(torch.utils.data.Dataset):
    ...
    def get_events_base_number(self, number_of_event: int,  t_end_us: int) -> Dict[str, np.ndarray]:
        ...
        events = load_events_with_retry(self.event_root, t_start_us_idx, t_end_us_idx, self.t_offset)
        return events
    ...
    def get_conservative_window_ms(ts_start_us: int, ts_end_us) -> Tuple[int, int]:
        ...
        window_start_ms = math.floor(ts_start_us / 1000000)
        window_end_ms = math.ceil(ts_end_us / 1000000)
