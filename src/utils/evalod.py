...
def kitti_output(box_pred, image_index, output_path, box_pred_right=None):
    with open(os.path.join(output_path, '{:06d}.txt'.format(image_index)), 'w') as f:
        for prediction in box_pred:
            for i, (cls, bbox, score) in enumerate(zip(
                ...
                cls_type = 'Pedestrian' if cls == 2 else 'Car' if cls == 1 else 'Cyclist'
